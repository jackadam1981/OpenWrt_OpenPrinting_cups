...
GitHub Actions 从 [downloads.openwrt.org](https://downloads.openwrt.org) 下载 OpenWrt 24.10 SDK 并编译，产出 `.ipk`。推送到 `main`/`master` 或提交 PR 时触发，产物作为 Artifact 提供下载。若仓库中存在 `templates/zh_CN`，构建时会将其中的 `*.tmpl` 复制到 CUPS 源码，使 Web 管理界面支持简体中文。
...
## Web 管理 ([http://IP:631](http://IP:631))

- 管理入口：**http://设备IP:631/admin**（需用 root 及 root 密码登录）。
- **简体中文界面**：本仓库提供 `templates/zh_CN` 下的 Web 模板。CI 构建时会将其中 `*.tmpl` 复制到 CUPS 源码的 `templates/zh_CN/`，随包一起安装。浏览器语言设为简体中文（zh-CN）时，CUPS Web 管理界面会自动使用中文；无需单独安装语言包。
...
| 步骤                | 命令             | 说明                                     |
| ----------------- | -------------- | -------------------------------------- |
| **检查 cupsd 是否运行** | `ps            | grep cupsd`或`/etc/init.d/cupsd status` |
| **检查端口 631 是否监听** | `netstat -tlnp | grep 631`或`ss -tlnp                    |
...
│   └── Makefile
├── templates/zh_CN/
│   ├── *.tmpl          # Web 管理界面简体中文模板（构建时复制到 CUPS 源码）
│   └── README.md
├── .github/workflows/
│   ├── build.yml       # 多架构构建（含复制 zh_CN 模板步骤）
│   └── update-check.yml
└── README.md
```
